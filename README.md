...
Your setup leaves your machine in exactly one place, and only when you
ask for it: clicking the footnote's "Wiki calc" button uploads that
shown setup (gear, levels, prayers, boosts, monster) to the wiki's own
share service to create the link it opens - exactly what the
calculator's Share button does.

The plugin also fetches monster pictures from the OSRS wiki for the mob
rows: one request per monster, using the file name the bundled monster
data already carries, cached on disk under
`.runelite/loadout-lab/npc-icons/`. Nothing about you rides along.
